...
const generatedMarker = "Generated by ci-resolver"
...
// must not leave an orphan workflow behind). Without the orphan sweep the "codegen
// can't silently lie" guarantee would leak once one DAG maps to many files.
func checkFreshDir(dir, ext string, want map[string][]byte) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil // nothing committed yet — the per-file checks above already failed
		}
		return err
	}
	for _, e := range entries {
		...
		if _, ok := want[path]; ok {
			continue // an expected file, already verified above
		}
		...
		if bytes.Contains(data, []byte(generatedMarker)) {
			return fmt.Errorf("freshness check: %s is an orphan — no goal produces it; "+
				"remove the file or restore the goal", path)
		}
	}
	return nil
}
